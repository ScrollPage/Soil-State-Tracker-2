...
class TestViews(APITestCase):
    ...
    def test_chat_delete_user(self):
        response = get_response(
            'chat-detail', 'delete', self.user1, 
            kwargs={'pk': self.chat1.id}
        )
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

    def test_chat_delete_admin(self):
        response = get_response(
            'chat-detail', 'delete', self.user4, 
            kwargs={'pk': self.chat1.id}
        )
    ...
    def test_chat_admin_by_user(self):
        response = get_response(
            'chat-admin', 'put', self.user1, 
            kwargs={'pk': self.chat2.id}
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_chat_admin_exists(self):
        response = get_response(
            'chat-admin', 'put', self.user4, 
            kwargs={'pk': self.chat1.id}
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
