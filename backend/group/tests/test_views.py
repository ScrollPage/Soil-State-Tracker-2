...
class TestViews(APITestCase):
    def setUp(self):
        ...
        self.cluster_detector = Detector.objects.create(
            user=self.user1, x=1, y=1, cluster=self.cluster
        )
    ...
    def test_cluster_detectors_list(self):
        response = get_response(
            "cluster-detectors", "get", self.user1, kwargs={"pk": self.cluster.id}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
    ...
    def test_cluster_detectors_list_wrong_cluster(self):
        response = get_response(
            "cluster-detectors", "get", self.user1, kwargs={"pk": 0}
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_cluster_delete(self):
        response = get_response(
            "cluster-detectors", "delete", self.user1, kwargs={"pk": self.cluster.id}
        )
    ...
    def test_cluster_update(self):
        data = {"name": "123123"}
        response = get_response(
            "cluster-detectors", "put", self.user1, data, {"pk": self.cluster.id}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_cluster_detectors_add(self):
        response = get_response(
            ...
            "post",
            self.user1,
            {"detectors": [self.free_detector.id]},
            {"pk": self.cluster.id},
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = get_response(
    ...
    def test_cluster_detectors_remove(self):
        response = get_response(
            ...
            "post",
            self.user1,
            {"detectors": [self.cluster_detector.id]},
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = get_response(
            "cluster-detectors", "get", self.user1, kwargs={"pk": self.cluster.id}
